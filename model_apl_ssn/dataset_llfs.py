...
class Dataset(td.Dataset):
    ...
    def __getitem__(self, idx):        
        while True:
            ...
            llfs_segment = llfs_combined[segment_start_idx:segment_start_idx + self.n_frames, :] #(N, 96)
           
            break
        
        return (mel_segment, lm_data_list, llfs_segment, emo_tensor, os.path.join(lm_folder,lm_paths[segment_start_idx + (self.n_frames + 1)//2-1]))

    def collate_fn(self, batch):
        batch_audio, batch_landmark, batch_llfs, batch_emo, lm_paths = zip(*batch)
        ...
        return batch_audio, batch_landmark, batch_llfs, batch_emo, lm_paths
