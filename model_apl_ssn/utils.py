...
def convert_emo_cremad_to_one_hot(emotion_label, emotion_level):
    num_classes = len(emo_cremad_to_idx)
    emotion_idx = emo_cremad_to_idx[emotion_label]
    one_hot_vector = torch.zeros(num_classes + 3) 
    one_hot_vector[emotion_idx] = 1 
    emotion_level_idx = emo_cremad_level[emotion_level]
    if emo_cremad_to_idx != 0:
        one_hot_vector[num_classes + emotion_level_idx - 1] = 1
    return one_hot_vector