...
class Model(nn.Module):
    ...
    def loss_fn(self, y_pred, y):
        ce_loss_emotion = self.emo_lb_ce_loss(F.softmax(y_pred[:, :8], dim=1), y[:, :8])
        ce_loss_level = self.emo_lv_ce_loss(F.softmax(y_pred[:, 8:], dim=1), y[:, 8:])
