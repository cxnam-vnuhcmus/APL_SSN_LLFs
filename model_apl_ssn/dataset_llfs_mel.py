...
class Dataset(td.Dataset):
    ...
    def __getitem__(self, idx):        
        while True:
            ...
            llfs_db_delta = torch.tensor(data["llfs_db_delta"])
            llfs_db_delta2 = torch.tensor(data["llfs_db_delta2"])
            llfs_combined = torch.cat((llfs, llfs_db_delta, llfs_db_delta), axis=1)
            #landmark
            lm_paths = sorted(os.listdir(lm_folder))
            ...
            mfcc_segment = mfcc[segment_start_idx:segment_start_idx + self.n_frames, :] #(N, 80)
            ...
            max_len = llfs.shape[0]
            if max_len < 30:
                idx = idx + 1
                continue
            left = max(0, segment_start_idx - 15)
            right = min(left + 30, max_len)
            left = left - (left + 30 - right)
            llfs_segment = llfs_combined[left:right, :] #(N, 96)
            mel_segment = mel_spectrogram_db[left:right, :]
            break
        
        return (mfcc_segment, lm_data_list, mel_segment, llfs_segment, emo_tensor, os.path.join(lm_folder,lm_paths[segment_start_idx + (self.n_frames + 1)//2-1]))

    def collate_fn(self, batch):
        batch_audio, batch_landmark, batch_mel, batch_llfs, batch_emo, lm_paths = zip(*batch)
        ...
        if not all(au is None for au in batch_mel):
            batch_mel = [batch_mel[idx] for idx in keep_ids]
            batch_mel = torch.stack(batch_mel)
        else:
            batch_mel = None
        ...
        return batch_audio, batch_landmark, batch_mel, batch_llfs, batch_emo, lm_paths
